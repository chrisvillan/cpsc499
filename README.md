...
Project Source Code: https://github.com/chrisvillan/CPSC499
...
This logic can help in distinguishing the true key between relative keys. For example, in the song “Greensleeves” by Yasunori Mitsuda, the first line of notes are E, G, A, B, C, B, A, F#, D, E, F#, G, E.
 
When the notes are being analyzed, the 8th note will be an F#. This is the first clue to see to determine what key it is. A scale with an F# can either be an E Minor or G Major. The way to determine which the song is lies on the next line. The notes are E, D#, E, F#, D#, B, E, G, A, B, C, B, A, F#. 

On the next line, the second note has a D# which isn’t set in the original key. If we determine an E Minor chord and a G major chord, an E Minor chord has the notes E, G, and B. For the G major chord, the notes are G, B, and D. Since the G major chord has a note D in its triad form, that D is crucial for that chord which translates to the key. Therefore, if the song was in the key of G major, the D can’t turn into a D#. In conclusion, the song is in E Minor, which is the true key of the song. 

The flaw in the above algorithm is that it depends on a unique alteration to determine the key of a song. For the song “Under The Sea” in Walt Disney’s Little Mermaid, there is no altered notes outside the given key. Therefore, the algorithm wouldn’t be able to determine between the relative keys. There is another way of choosing between relative keys and that’s understanding what the purpose of the root note is. 

The root note sets the basis of the song and sets the tonal center. The tonal center is the note where other notes surround it, which usually is the root note. In most songs the root note of the scale it is in, is one of the most played notes in the song. That concept will be the solution that overcomes the previous problem. For the song “Under The Sea”, we are able to identify the key signature and will need to choose between the relative keys. 
...
http://www.arttherapyblog.com/online/color-psychology-psychologica-effects-of-colors/#.Wvp7Gogvzb0
